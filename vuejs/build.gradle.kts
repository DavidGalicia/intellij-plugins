...
var ideaHomePath = "/home/user0/idea-IU-252.25557.131"
...
dependencies {
  intellijPlatform {
    ...
    testPlugins(listOf(
      "org.jetbrains.plugins.stylus:252.23591.19",
...
tasks {
  ...
  // Copy the Vue language server directory into the plugin root in the sandbox and distribution
  prepareSandbox {
    from("vue-language-server") {
      into("${pluginName.get()}/vue-language-server")
    }
  }
  // Ensure test sandbox also contains the language server for local runs/tests
  prepareTestSandbox {
    from("vue-language-server") {
      into("${pluginName.get()}/vue-language-server")
    }
...
sourceSets {
  main {
    ...
    resources {
      setSrcDirs(listOf("resources", "gen-resources", "vue-language-server"))
    }
  }
  test {
    java {
...
dependencies {
  ...
  testImplementation("com.jetbrains.intellij.platform:poly-symbols-test-framework:252.25557.131")
